/*
  ...
  Descrição: O link de navegação "Início" foi atualizado para apontar para a nova rota da página inicial de usuários logados (/app/home).
*/
...
const Sidebar = ({ isOpen, closeSidebar }) => {
    ...
    const isMapPage = location.pathname.startsWith('/app/mindmap');
    ...
    return (
        <aside id="app-sidebar" className={`w-64 p-4 space-y-4 glass-effect fixed top-0 md:top-16 left-0 h-full md:h-[calc(100vh-64px)] overflow-y-auto border-r border-gray-200/80 ${isOpen ? 'sidebar-mobile-open' : ''}`}>
            <nav className="flex flex-col space-y-2">
                <NavLink to="/app/home" className="sidebar-link" onClick={handleLinkClick}>
                    <span className="material-icons sidebar-icon mr-3">home</span>
                    Início
                </NavLink>
